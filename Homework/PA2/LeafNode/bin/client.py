...
def client():
	...
	def getFile(serverAddr, serverPort):
			try: 
			...
			if filesize == 0:
				print("- Peer Client: Requested file was not found on peer server: " + serverPort)
				return
	
	
			#Download file
			with open(globals.LOCAL_SHARED_FILE_SPACE + "\\" + requestedFile, 'wb') as file:
				while True:
					try:
	...
	def getCatalog(sock):
		...
		try: 
			clientConnect(sock)
			sock.send(pickle.dumps((globals.CATALOG, "search")))
			msg = "\n"
	...
	def clientSearch(sock, requestedFile, flag):
			...
			try: 
				clientConnect(sock)
				#search index server
				if globals.TOPOLOGY == "broadcast":
					pickled = pickle.dumps((globals.SEARCH_FILES_BROADCAST, requestedFile))
					sock.send(pickled)
				else:
					pickled = pickle.dumps((globals.SEARCH_FILES_LINEAR, requestedFile))
					sock.send(pickled)
			except socket.error as err:
				print("- Peer Client: failed to contact index server, aborting")
				return -1
			...
			if len(response) == 0:
				...
				node = random.randint(0, len(response)-1)
				serverAddr = response[node][0]
				serverPort = response[node][1]
				print("+ Peer Client: Looking for '" + str(requestedFile) + "' on: " + str(serverPort))
				getFile(serverAddr, serverPort)
			else:
				return 0
			return 1
	...
	while True:
		...
		if requestedFile == 'exit':
			...
			globals.TOPOLOGY = 'linear'
			
		elif requestedFile == 'broadcast':
			globals.TOPOLOGY = 'broadcast'
			
		elif requestedFile == 'topology':
			print("The current topology is " + globals.TOPOLOGY)
